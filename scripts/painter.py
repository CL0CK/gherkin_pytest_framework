...
class Painter:
    ...
    def draw_colored_line_in_rectangle(
        cls, ax: Axes, color: str, x_coords: list[int | float], y_coords: list[int | float],
    ) -> None:
        ax.add_line(Line2D(x_coords, y_coords, color=color, linewidth=3, solid_capstyle='round'))

    @classmethod
    def draw_duration_title_rectangle(
        cls, ax: Axes, x_coord: int | float, y_coord: int | float
    ) -> None:
    ...
    def configure_pie_description(cls, ax: Axes, data: dict, side: Literal['left', 'right'], colors: dict) -> None:
        ...
        rectangles_data: dict = {
            ...
            'large': {
                'durations': {
                    'rectangle_coords': [0, 1.82],
                    'title_coords': [2, 3.3],
                    'duration_coords': [2, 2.5]
                }
            }
        }
                    ...
                    for data_key, config in case_status_data.items():
                        if not isinstance(config, str):
                            if isinstance(config[0], list):
                                right_side_coords = [
                                    [config[0][0] + 4.05, config[0][1] + 4.05], [config[1][0], config[1][1]]
                                ]
                            else:
    ...
    def configure_single_pie(
        cls, data: dict, fig: plt.Figure, gs: plt.GridSpec, title: str, subplot: Axes, side: Literal['left', 'right']
    ) -> None:
        ax: Axes = fig.add_subplot(gs)  # type: ignore
        colors = {
            'passed': '#97cc64', 'expected_fail': '#d35ebf', 'broken': '#ffd050', 'failed': '#fc593e',
            'skipped': '#a8a8a8'
        }
    ...
    def create_statistic_image(cls, data: dict, platform: str, stream: str) -> str:
        ...
        cls.configure_single_pie(
            data['today'], fig, gs[0, 1], 'Current Test Result', subplot, 'right'  # type: ignore
        )
        if len(data) == 2:
            cls.configure_single_pie(
                data['previous'], fig, gs[0, 0], 'Previous Test Result', subplot, 'left'  # type: ignore
            )
